package com.ysxsoft.deliverylocker_big.widget.wheelview;
...
public class WheelRecyclerView<T extends IPickerViewData> extends RecyclerView {

    //默认参数
    private final int DEFAULT_WIDTH = DensityUtil.dp2px(MyApplication.getApplication(),160);

    private final int DEFAULT_ITEM_HEIGHT = DensityUtil.dp2px(MyApplication.getApplication(), 50);

    private final int DEFAULT_SELECT_TEXT_COLOR = Color.parseColor("#3396FF");

    private final int DEFAULT_UNSELECT_TEXT_COLOR = getResources().getColor(R.color.colorBlack);

    private final int DEFAULT_SELECT_TEXT_SIZE = DensityUtil.dp2px(MyApplication.getApplication(),14);

    private final int DEFAULT_UNSELECT_TEXT_SIZE = DensityUtil.dp2px(MyApplication.getApplication(), 14);

    private final int DEFAULT_OFFSET = 1;

    private final int DEFAULT_DIVIDER_WIDTH = ViewGroup.LayoutParams.MATCH_PARENT;

    private final int DEFAULT_DIVIDER_HEIGHT = DensityUtil.dp2px(MyApplication.getApplication(), 1);
    ...
    private int dividerMargin;//分割线距离两边边距
    ...
    public WheelRecyclerView(Context context, @Nullable AttributeSet attrs) {
        ...
        dividerMargin = (int)ta.getDimension(R.styleable.WheelRecyclerView_dividerMargin, 0);
    ...
    private class WheelAdapter extends Adapter<WheelAdapter.WheelHolder> {
        ...
        public WheelHolder onCreateViewHolder(ViewGroup parent, int viewType) {
            ...
            Log.e("Mywheelview", "onCreateViewHolder");
            return holder;
        }
        ...
        public void onBindViewHolder(WheelHolder holder, int position) {
            ...
            if (position < mOffset || position > mDatas.size() + mOffset - 1) {
                ...
                holder.name.setText(mDatas.get(position - mOffset).getPickerViewText());
            }
            if (position == mOffset){//在view高度设置为固定值或者指定高度时(非固定高度也有这种情况)，notifydatasetchange会重构当前页面item 调用onCreateViewHolder初始化item，这里初始化设置一下
                holder.name.setTextSize(TypedValue.COMPLEX_UNIT_PX, mSelectTextSize);
                holder.name.setTextColor(mSelectTextColor);
                holder.name.setTypeface(Typeface.defaultFromStyle(Typeface.BOLD));
            }else {
                holder.name.setTextSize(TypedValue.COMPLEX_UNIT_PX, mUnselectTextSize);
                holder.name.setTextColor(mUnselectTextColor);
    ...
    private void setSelectedItem(){
        ...
        for (int i = 0; i < 1 + mOffset * 2; i++) {
            ...
            if (item == null) return;
            if (i == mOffset) {//因为第一个位置是默认位置，所以i==0更改item状态
    ...
    private class DividerItemDecoration extends ItemDecoration {
        ...
        public void onDrawOver(Canvas c, RecyclerView parent, State state) {
            //绘制分割线
            float startX = getMeasuredWidth() / 2 - mDividerWidth / 2 + dividerMargin;
            float topY = mItemHeight * mOffset;
            float endX = getMeasuredWidth() / 2 + mDividerWidth / 2 - dividerMargin;
    ...
    public interface OnSelectListener<T> {
        void onSelect(int position, T data);
    }
